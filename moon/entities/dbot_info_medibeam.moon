...
ENT.Type = 'anim'
ENT.Base = 'base_anim'
ENT.PrintName = 'Medigun beam effect'
ENT.Spawnable = false
...
ENT.Initialize = =>
	if SERVER
		...
		@SetModel('models/props_junk/popcan01a.mdl')
		@DrawShadow(false)
		@beamSound = CreateSound(@, 'DTF2_WeaponMedigun.Healing')
		@beamSound\Play()
...
if CLIENT
	translucentMaterual = CreateMaterial('DTF2_Translucent_MediBeam', 'VertexLitGeneric', {
		'$translucent': '1'
		'$alpha': '0'
		'$color': '[0 0 0]'
	})
	...
	ENT.Draw = =>
		...
		if IsValid(@particleEffect)
			with @GetDummyTarget()
				...
				\DrawShadow(false)
				\SetModelScale(0.01)
				\SetMaterial('!DTF2_Translucent_MediBeam')
			return
		...
		@particleEffect = @CreateParticleEffect(@GetBeamType() and 'medicgun_beam_blue_healing' or 'medicgun_beam_red_healing', {pointOne, pointTwo})
