...
function Hist.Draw()
	...
	for k, v in pairs(Hist.Weapons) do
		DHUD2.DrawBox(x + v.shift, y + ShiftY, WIDTH, 24, bg)
		DHUD2.DrawBox(x - 24 + v.shift, y + ShiftY, 24, 24, weapon)
		DHUD2.SimpleText(v.name, nil, x + 5 + v.shift, y + 2 + ShiftY)
		ShiftY = ShiftY + 24
	end
	
	for k, v in pairs(Hist.Items) do
		DHUD2.DrawBox(x + v.shift, y + ShiftY, WIDTH, 24, bg)
		DHUD2.DrawBox(x - 24 + v.shift, y + ShiftY, 24, 24, item)
		DHUD2.SimpleText('#' .. v.name, nil, x + 5 + v.shift, y + 2 + ShiftY)
		ShiftY = ShiftY + 24
	end
	...
	for k, v in pairs(Hist.Ammos) do
		...
		DHUD2.DrawBox(x + v.shift, y + ShiftY, WIDTH, 24, bg)
		DHUD2.DrawBox(x - 24 + v.shift, y + ShiftY, 24, 24, ammo)
		DHUD2.SimpleText(name, nil, x + 5 + v.shift, y + 2 + ShiftY)
		
		DHUD2.SimpleText(' (' .. v.amount .. ')', nil, x + 10 + v.shift + w, y + 2 + ShiftY)
